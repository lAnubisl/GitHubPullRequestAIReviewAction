...
ILocalFileContextReader localFileContextReader = new LocalFileContextReader();
IGitHubContextReader contextReader = new GitHubContextReader(configuration);
IGitHubPullRequestService pullRequestService = new GitHubPullRequestService(commandRunner, configuration, diffParser, localFileContextReader, logger);
